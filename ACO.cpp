...
//taumax -> pheromone trail constant
//ro-> coeff of vapourization of pheromone
//alpha and beta are constants
	
#define	OPT_ED_RATIO (double)0.5
...
void ACO::setCITYPOSITION (int city, double x, double y,double z) 
{
...
{
	return (double) 
		sqrt (pow (CITIES[cityi][0] - CITIES[cityj][0], 2) +pow (CITIES[cityi][1] - CITIES[cityj][1], 2));
...
int ACO::valid (int antk, int iteration) 
{
	...
	for(int i=0; i<NUMBEROFCITIES-1; i++) 
	{
		...
		if(CITIES[cityj][2]<=OPT_ED_RATIO)
			return-4;
...
void ACO::printRESULTS () {
	...
	cout << endl << "Data Successfully Transmitted";
  ...
  cout << endl << "Path length: " << BESTLENGTH;
  cout << endl << "Total Energy Consumption ratio :" << BESTRATIO<< endl;
...
void ACO::optimize (int ITERATIONS,int dest) 
{
	for (int iterations=1; iterations<=ITERATIONS; iterations++) 
	{
		...
		for (int k=0; k<NUMBEROFANTS; k++) 
		{
			...
			double rlength = length(k);
			double rratio = ratio(k);
			if (rlength < BESTLENGTH && ROUTES[k][NUMBEROFCITIES-1]==dest)
			 {
				BESTLENGTH = rlength;
				BESTRATIO = rratio;
				for (int i=0; i<NUMBEROFCITIES; i++) 
